# AI-Based Demand Forecasting & Inventory Optimization Platform for Retail and Supply Networks
...
Retail businesses often face costly challenges such as **overstocking**, **stockouts**, and **poor demand estimation**. NeuralStock addresses these problems by providing:
...
2. **Actionable Inventory Math:** Translating demand predictions into real-world purchasing policies (Economic Order Quantity & Reorder Points).  
...
- **Quality & Risk Index:** A combined metric that evaluates the operational risk of using forecasts for inventory planning.  
...
- Instantly calculates **Economic Order Quantity (EOQ)**.  
- Calculates **Safety Stock** and **Reorder Points** using an AI-driven service level of 95% (z-score = 1.65).  

---

## 🎨 UI / UX Design

The frontend has been completely overhauled to feature a sophisticated, AI-themed dark aesthetic:
...
Run the setup script to create the SQLite database and seed it with historical sales data.
...
- **Features Used:**  
`Store`, `Dept`, `year`, `month`, `week`, `day`, `dayofweek`, `quarter`, `store_dept`, `month_squared`, `sales_lag1`, `sales_lag4`, `sales_rolling_mean`, `sales_rolling_std`

- **Inference Strategy:**  
When making a new prediction, the system retrieves the most relevant recent historical data for the corresponding Store and Department to dynamically populate the lag and rolling mean features.